...
class DatasetCollection:
    ...
    def init(self):
        if self.type == "Imagenet":
            ...
            train_dataset = datasets.ImageFolder(traindr, self.compose["train"])
            val_dataset = datasets.ImageFolder(valdr, self.compose["val"])
            ...
            #     download_url(url, self.root, filename, tgz_md5)
            images = pd.read_csv(
                os.path.join(self.path, "CUB_200_2011", "images.txt"),
                sep=" ",
                names=["img_id", "filepath"],
            )
            image_class_labels = pd.read_csv(
                os.path.join(self.path, "CUB_200_2011", "image_class_labels.txt"),
                sep=" ",
                names=["img_id", "target"],
            )
            train_test_split = pd.read_csv(
                os.path.join(self.path, "CUB_200_2011", "train_test_split.txt"),
                sep=" ",
