...
def main_worker(rank, process_num, args):
    ...
    conv1 = nn.Conv2d(8, 8, (1, 4), (1, 4)).to(rank)
    tconv1 = nn.ConvTranspose2d(8, 8, (1, 4), (1, 4)).to(rank)
    conv2 = nn.Conv2d(8, 8, (1, 4), (1, 4)).to(rank)
    tconv2 = nn.ConvTranspose2d(8, 8, (1, 4), (1, 4)).to(rank)
    conv1 = torch.nn.parallel.DistributedDataParallel(conv1)
    tconv1 = torch.nn.parallel.DistributedDataParallel(tconv1)
    conv2 = torch.nn.parallel.DistributedDataParallel(conv2)
    tconv2 = torch.nn.parallel.DistributedDataParallel(tconv2)
    optimizer = AdamW(
        [
            {"params": conv1.parameters()},
            {"params": tconv1.parameters()},
            {"params": conv2.parameters()},
            {"params": tconv2.parameters()},
        ],
    )
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            outputs = conv1(labels1)
            # print(outputs.shape)
            outputs1 = tconv1(outputs)
            # print(outputs1.shape)
            labels2 = part2(labels1, batch["attention_mask"])
            outputs = conv2(labels2)
            outputs2 = tconv2(outputs)
            ...
            torch.save(conv1.module.state_dict(), str(args.rank) + "_conv1.pth")
            torch.save(tconv1.module.state_dict(), str(args.rank) + "_tconv1.pth")
            torch.save(conv2.module.state_dict(), str(args.rank) + "_conv2.pth")
            torch.save(tconv2.module.state_dict(), str(args.rank) + "_tconv2.pth")
