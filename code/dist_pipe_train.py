...
def main_worker(rank, world_size, args):
    ...
    if rank == 0:
        ...
        for epoch in range(args.epochs):
            batch_time1, data_time1, acc1_avg, loss_avg = train_header(
                rank, model1, model2, optimizer, train_loader, criterion, args
            )
            ...
            batch_time1, val_acc1, val_loss = val_header(
                rank, model1, model2, val_loader, criterion, args
            )
            ...
            file_save1.write(
                ...
                + str(val_acc1.item())
                + "  time_per_batch:"
                + str(train_batch_time_avg)
                + "  time_load_perbatch:"
                + str(train_data_time_avg)
            )
        ...
        for epoch in range(args.epochs):
            batch_time, data_time = train_medium(
                rank, model, optimizer, len(train_loader), args
            )
        ...
        warmup_schduler = warmup.LinearWarmup(optimizer, warmup_period=10)
        for epoch in range(args.epochs):
            batch_time3, data_time3 = train_last(
                rank, model, optimizer, len(train_loader), args
            )
