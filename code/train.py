...
layer1 = layer1.to(0)
layer2 = nn.Sequential(DeQuantLayer(16),model.layers[0:10])
DeferredBatchNorm.convert_deferred_batch_norm(layer2,4)
layer2 = layer2.to(1)
layer3 = nn.Sequential(model.layers[10:],QuantLayer(16))
DeferredBatchNorm.convert_deferred_batch_norm(layer3,4)
layer3 =layer3.to(2)
layer4 = nn.Sequential(DeQuantLayer(16),model.conv2,model.bn2,Reshape1(), model.linear)
...
model = GPipe(model,partition = partition,devices = [0,1,2,0], chunks=4)
# model = model.to(0)
