...
class QuantFunction(autograd.Function):
    ...
    def forward(ctx,input,bits):
        ...
        output = torch.cat([torch.floor((input.view(-1)-min)/step),step,min,shape],0)
        print("quant forward")
        return output
    @staticmethod
    def backward(ctx,grad_output):
...
class DeQuantFunction(autograd.Function):
    ...
    def backward(ctx,grad_output):
        ...
        step = torch.tensor([(max - min) / (pow(2,ctx.bits)-1)]).to(grad_output.get_device())
        min = torch.tensor([min.item()]).to(grad_output.get_device())
        shape = torch.tensor(grad_output.shape).to(grad_output.get_device())
        output = torch.cat([torch.floor((grad_output.view(-1)-min)/step),step,min,shape],0)
        return output,None
