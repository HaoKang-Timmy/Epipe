...
parser.add_argument(
    "--world-size", default=2, type=int, help="number of nodes for distributed training"
)
parser.add_argument(
...
def main_worker(rank, world_size, args):
    ...
    if rank == 0:
        ...
        optimizer = SGD(param_list, lr=0.005, weight_decay=0.0)
        ...
        for epoch in range(args.epochs):
            ...
            for batchiter, (image, label) in enumerate(train_loader):
                ...
                for chunk in range(args.chunk):
                    recv = (
                        torch.zeros([int(args.batches / args.chunk), 1280, 7, 7])
                        .to(0)
                        .requires_grad_()
                    )
                    # dist.recv(recv,1)
                ...
                for chunk in range(args.chunk):
                    torch.zeros([])
                    batch1[chunk].backward(
                        torch.zeros([int(args.batches / args.chunk), 32, 112, 112]).to(
                            0
                        )
                    )
                optimizer.step()
        ...
        optimizer = SGD(layer2.parameters(), lr=0.005, weight_decay=0.0)
        for epoch in range(args.epochs):
            layer2.train()
            for batch_iter in range(len(train_loader)):
                ...
                #     pass
                for chunk in range(args.chunk):
                    batch[chunk].backward(
                        torch.zeros(tuple(list(batch[chunk].shape))).to(rank)
                    )
                optimizer.step()
                optimizer.zero_grad()
