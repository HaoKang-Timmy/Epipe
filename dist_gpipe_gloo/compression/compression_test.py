...
"""
from torch import bitwise_left_shift as bls
from torch import bitwise_right_shift as brs
...
def q10toint32(input:torch.tensor):
    # input with a tensor just use 10bits int32 to zip int32
    zip_time = 3
    bitsize = 10
    output =input.view(-1)
    output = output.type(torch.int32)
    if output.shape[0] % zip_time != 0:
        shape = output.shape[0]+ zip_time - output.shape[0] % zip_time
        fix = torch.zeros([shape - output.shape[0]]).type(torch.int32).to(input.get_device())
        output = torch.cat((output,fix),0)
    output = output.view(-1,3)
    send = torch.zeros([output.shape[0]]).type(torch.int32).to(input.get_device())
    send[:] = output[:,0] + bls(output[:,1],bitsize) + bls(output[:,2],2 * bitsize)
    return send
def int32toq10(input:torch.tensor,recv:torch.tensor):
    # input size m*n output m*n
    shape = input.shape
    zip_time = 3
    bitsize = 10
    input = input.view(-1)
    length = input.shape[0]
    # viewshape =recv.shape
    if input.shape[0] % zip_time != 0:
        viewshape = input.shape[0]+ zip_time - input.shape[0] % zip_time
    # print(shape)
        fix = torch.zeros([viewshape - input.shape[0]]).type(torch.int32).to(input.get_device())
        input = torch.cat((input,fix),0)
    input = input.view(-1,3)
    input[:,1] = brs(bls(recv[:],1*bitsize+2),2*bitsize+2)
    input[:,2] = brs(recv[:],2*bitsize)

    input[:,0] = brs(bls(recv[:],2*bitsize+2),2*bitsize+2)
    input =input.view(-1)
    input =input[:length]
    input = input.view(shape)
    return input
    ...
    pass
def q12toint32(input:torch.tensor):
    zip_time = 5#64/12
    bitsize = 12
    output =input.view(-1)
    ...
    if output.shape[0] % zip_time != 0:
        ...
        fix = torch.zeros([shape - output.shape[0]]).type(torch.int32).to(input.get_device())
        output = torch.cat((output,fix),0)
    output = output.view(-1,zip_time)
    send = torch.zeros([output.shape[0]]).type(torch.int64).to(input.get_device())
    send[:] = output[:,0] + bls(output[:,1],bitsize) + bls(output[:,2],2 * bitsize)
    pass
...
def DequantizationonGPU(input,bits,min,step):
    # print("recv 16",input)
    output = ((input.type(torch.float32)+ pow(2, bits - 1))*step + min).requires_grad_()
    return output
...
class QSendLayerGPU(nn.Module):
    ...
    def forward(self, input):
        ...
        return QSendGPU.apply(
            input, self.bits, self.send_rank, self.rank
        )
def int32to10():
    pass
def int32to12():
        ...
        for i in range(2 ** split_bits):
            ...
            # print(i)
                ...
                if min != max:
                    ...
                    temp_src = torch.round((src[i] - min) / step) - pow(2,bits+split_bits - 1)
                    temp_src += pow(2,bits) * i
                    # temp_src += 
                else:
                    min_step[i,0],min_step[i,1] = min,0.0
                    temp_src = 1 - pow(2,bits+split_bits - 1)
                    temp_src += pow(2,bits) * i
            else:
                min, max = src[i].min(),src[i].max()
                if min != max:
...
class SortDeQuantGPU(autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.view(shape)
        # print(grad_output.shape)
        return grad_output.requires_grad_(),None,None,None
