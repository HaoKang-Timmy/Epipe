...
'''
import torch
from transformers import AutoModelForSequenceClassification,RobertaTokenizer,DataCollatorWithPadding
...
def main():
    ...
    tokenized_cola = tokenized_cola.map(lambda examples: {'labels': examples['label']}, batched=True)

    tokenized_cola.set_format(type='torch', columns=['input_ids', 'attention_mask',  'labels'])
    # print(tokenized_cola[0])
    dataloader = torch.utils.data.DataLoader(tokenized_cola, batch_size=128, shuffle=True)
