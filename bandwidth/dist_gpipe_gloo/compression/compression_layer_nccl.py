...
def QtensorSendonGPUSync(input, min, step, send_rank, pg=None):
...
class SortQuantGPUSync(autograd.Function):
    @staticmethod
    def forward(ctx, input, bits, split_bits, send_rank, bandwidth, pg=None):
        ctx.recv_rank, ctx.bits, ctx.split_bits, ctx.pg = (
            send_rank,
            bits,
            split_bits,
        ...
        if bits + split_bits > 8 and bits + split_bits <= 16:
            ...
            output = output.view(dtype=torch.int8)
        start = time.time()
        dist.send(min_step, send_rank, group=pg)
        dist.send(output, send_rank, group=pg)
        end = time.time() - start
        bandwidth[0] = output.element_size() * output.nelement() / end
        input = input.view(shape)
        return input
...
class SortDeQuantGPUSync(autograd.Function):
    @staticmethod
    def forward(ctx, input, bits, split_bits, recv_rank, bandwidth, pg=None):
        ctx.send_rank, ctx.bits, ctx.split_bits, ctx.pg = (
            recv_rank,
            bits,
            split_bits,
